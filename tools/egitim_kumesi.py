...
egitim_kumesi.py — segmentler.csv + WAV'lardan model girdisi uret. (M4 adim 4)
    ...
    python tools/egitim_kumesi.py --saglama     # ONCE BUNU CALISTIRIN
    python tools/egitim_kumesi.py
...
tools/esc50_indir.py, `chirping_birds` cikarilmis). Negatif klipler
...
def main():
    ...
    if not os.path.exists(seg):
        sys.exit(f"{seg} yok — once tools/birdnet_ozet.py")
...
def negatif_listesi(a, ad_sinif):
    ...
    if not os.path.exists(kayit):
        print(f"\n!! NEGATIF SINIF BOS: {kayit} yok.\n"
              f"   Once:  python tools/esc50_indir.py\n"
