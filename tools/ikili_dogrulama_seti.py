...
tools/dogrulama_seti.py'nin (M6) ikili ag karsiligi — ayni gerekce: mikrofon/
...
Cikti: src/ai/ikili_dogrulama_seti.h
...
Kullanim:
    .venv-birdnet\\Scripts\\python tools/ikili_dogrulama_seti.py
"""
...
def main() -> None:
    ...
    if not MODEL.exists():
        sys.exit(f"{MODEL} yok — once tools/ikili_egit.py calistirin.")
    ...
    with open(CIKTI, "w", encoding="utf-8", newline="\n") as f:
        f.write(f"""/* Uretilmis dosya — tools/ikili_dogrulama_seti.py. ELLE DUZENLEMEYIN.
 *
 ...
 */
#ifndef POKEBIRD_IKILI_DOGRULAMA_SETI_H
#define POKEBIRD_IKILI_DOGRULAMA_SETI_H

#include <stdint.h>

#define PB_IKILI_DOGRULAMA_ADET  {len(secim)}
#define PB_IKILI_DOGRULAMA_KARE  {KARE}
#define PB_IKILI_DOGRULAMA_BANT  {BANT}

/* Gercek ikili etiket: 1 = KUS, 0 = DEGIL. */
static const int16_t pb_ikili_dogrulama_gercek[PB_IKILI_DOGRULAMA_ADET] = {{
{dizi(gercek_ikili)}
}};

/* PC REFERANS cekirdegin urettigi ham int8 logit (sigmoid oncesi). */
static const int8_t pb_ikili_dogrulama_logit[PB_IKILI_DOGRULAMA_ADET] = {{
{dizi(logitler)}
}};

/* Girdi pencereleri: kare disar (eskiden yeniye), bant icerde. */
static const int8_t pb_ikili_dogrulama_girdi[PB_IKILI_DOGRULAMA_ADET]
                                            [PB_IKILI_DOGRULAMA_KARE * PB_IKILI_DOGRULAMA_BANT] = {{
""")
