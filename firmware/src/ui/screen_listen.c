 ...
 */
#define BUTTON_W      96
#define INNER_X         102                 /* içerik sol kenarı               */
#define INNER_RIGHT       378                 /* içerik sağ kenarı               */
...
static lv_obj_t *s_name, *s_latin, *s_percent, *s_bar, *s_bar_yatak, *s_ayrac;
...
{
    lv_obj_t *all[] = { s_name, s_latin, s_percent, s_bar, s_bar_yatak,
                          s_ayrac, s_bottom[0], s_bottom[1],
...
lv_obj_t *pb_screen_listen_create(void)
{
    ...
    s_bar_yatak = pb_box(s_screen, INNER_X, BAR_Y, BAR_W, BAR_H,
                            PB_COLOR_ROW, 3);
    s_bar = pb_box(s_screen, INNER_X, BAR_Y, 0, BAR_H, PB_COLOR_ACCENT, 3);

    s_ayrac = pb_box(s_screen, INNER_X, 90, BAR_W, 1, PB_COLOR_ROW, 0);
...
void pb_screen_listen_set_recording(bool recording)
{
    ...
    const uint32_t cerceve = recording ? PB_COLOR_RECORD : PB_COLOR_BORDER;
    const uint32_t text    = recording ? PB_COLOR_RECORD : PB_COLOR_MUTED;

    lv_obj_set_style_border_color(s_button, lv_color_hex(cerceve), LV_PART_MAIN);
...
{
    if (!g || !s_screen) return;

    const bool species      = (g->mode == PB_DECISION_SPECIES);
    const bool unsure = (g->mode == PB_DECISION_UNSURE);
    const bool ses      = (g->mode == PB_DECISION_SOUND);
    ...
    /* ── Sonuç var mı ── */
    const int list = (g->top3_name[0] != NULL) ? 1 : 0;
    if (list != s_last_list) {
        s_last_list = list;
        result_show(list == 1);
    ...
    /* ── 1. tahmin ── */
    char buf[80];
    pb_text_upper(g->top3_name[0], buf, sizeof(buf));
    ...
    pb_write(s_latin, s_last_latin, sizeof(s_last_latin),
           g->top3_latin[0] ? g->top3_latin[0] : "");

    char y[12];
    snprintf(y, sizeof(y), "%d%%", (int)(g->top3_probability[0] * 100.0f + 0.5f));
    ...
    int32_t w = (int32_t)(g->top3_probability[0] * (float)BAR_W + 0.5f);
    if (w > BAR_W) w = BAR_W;
    if (w != s_last_bar_w) { s_last_bar_w = w; lv_obj_set_width(s_bar, w); }

    /* ── 2. ve 3. tahmin ── */
    for (int i = 0; i < 2; i++) {
        const char *raw = g->top3_name[i + 1];
        char row[80];
        if (raw) {
            char b2[64];
        ...
        if (raw) snprintf(y2, sizeof(y2), "%d%%",
                          (int)(g->top3_probability[i + 1] * 100.0f + 0.5f));
        else     y2[0] = '\0';
